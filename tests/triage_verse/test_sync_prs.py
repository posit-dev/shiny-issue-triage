...
def test_parse_pr_node_closed_unmerged():
    node = _pr_node(8, "2026-06-04T00:00:00Z", state="CLOSED", merged=False,
                    mergedAt=None, closingIssuesReferences={"nodes": []})
...
def test_incremental_pr_sync_stops_at_cursor(tmp_path):
    ...
    page = {"repository": {"pullRequests": {
        ...
        "nodes": [_pr_node(3, "2026-06-03T00:00:00Z"),
    ...
    numbers = {r["number"] for r in
               con.execute("SELECT number FROM issues WHERE is_pr=1")}
    assert numbers == {3}
    assert con.execute("SELECT COUNT(*) FROM prs").fetchone()[0] == 1
