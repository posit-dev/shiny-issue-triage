...
1. **Snapshot publish/bootstrap has never run against real GitHub.** `src/triage_verse/snapshot.py` is fully unit-tested with faked `gh`, and every `gh release` flag was verified against `gh --help`, but unlike `sync` (which had a live smoke test against `rstudio/reactlog`), the release upload/download path has not touched real GitHub. **Action:** before the first full-fleet blitz, run `uv run triage-verse snapshot publish --dated` then `triage-verse snapshot bootstrap --force` on a scratch checkout and confirm the round-trip. The Task 12 runbook notes this too. Risk if skipped: a flag mismatch surfaces during the first real publish rather than now. Low effort, ~5 min.
